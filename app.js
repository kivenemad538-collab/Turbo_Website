...
function renderOffline(msg='الموقع جاهز، لكن رابط Railway لسه محتاج يتضاف في config.js.'){
  ...
  $('#rulesList').innerHTML='<div class="notice">القوانين هتظهر تلقائيًا بعد اتصال الموقع بـ Railway.</div>';
...
function renderPublic(){
  ...
  $('#rulesList').innerHTML=pub.settings.rules.length?pub.settings.rules.map((r,i)=>`<div class="rule"><b>${String(i+1).padStart(2,'0')}</b><span>${esc(r)}</span></div>`).join(''):'<div class="notice">الإدارة لسه ما أضافتش قوانين من لوحة التحكم.</div>';
...
function renderMe(){
  ...
  if(me.isAdmin){$('#adminLink').classList.remove('hidden');$('#admin').classList.remove('hidden');renderAdmin()}
