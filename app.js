...
function filterApplications(){
  ...
  const rows=[...adminState.applications].reverse().filter(a=>!q||String(a.realName||'').toLowerCase().includes(q)||String(a.discordId||'').includes(q)||String(a.discordTag||'').toLowerCase().includes(q)||String(a.number).includes(q));
  const list=$('#applicationList');if(list)list.innerHTML=rows.map(applicationRow).join('')||'<div class="notice">مفيش نتائج مطابقة.</div>';
}
...
async function renderAdmin(){
  ...
  $('#adminBox').innerHTML=`<div class="admin-dashboard">
    ...
    <div class="card applications-card"><div class="applications-toolbar"><div><span>APPLICATION REVIEW</span><h3>مراجعة التقديمات</h3></div><div class="application-search"><input id="applicationSearch" placeholder="ابحث بالاسم أو Discord ID أو رقم التقديم" oninput="filterApplications()"><span>⌕</span></div></div><div id="applicationList" class="application-list"></div></div>
